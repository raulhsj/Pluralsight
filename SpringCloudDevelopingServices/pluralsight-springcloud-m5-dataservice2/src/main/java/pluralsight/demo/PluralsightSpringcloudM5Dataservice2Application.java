...
public class PluralsightSpringcloudM5Dataservice2Application {
	...
	public @ResponseBody String getCustomerVehicleDetails(@PathVariable Integer cid)  throws InterruptedException {
		...
		// Span s = this.tracer.nextcreateSpan("lookupvehicle");
		Span s = this.tracer.nextSpan().name("lookupvehicle");
		try {
			//add tags
			// this.tracer.currentSpan().addTag("customerid", cid.toString());
			s.customizer().tag("customerid", cid.toString());
			
			// s.logEvent("DB query started");
			s.annotate("DB query started");
